...
embedding_size, max_sentence_len, num_classes, class_names, dst_train, dst_test, testloader = get_dataset("SST2-transformer", "")

net = get_network("LSTMNet", embedding_size, num_classes).to(device)
# net = get_network("MLP", embedding_size, num_classes).to(device)
optim = torch.optim.Adam(net.parameters(), lr=0.0001, weight_decay=0.05)
